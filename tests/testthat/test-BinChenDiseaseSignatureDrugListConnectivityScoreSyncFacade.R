...
# when
result <- sut$compute(disease_signature, drug_signatures, length(LINCS_bing), random_distribution_size = 10, disease_name = "IPF")
result$p.value <- NULL
result$adj.p.value <- NULL

# then
test_that("test-BinChenDiseaseSignatureDrugListConnectivityScoreSyncFacade", {

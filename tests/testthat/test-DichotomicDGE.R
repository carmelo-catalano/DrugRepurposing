library(testthat)

# setup
meanThresholdGeneFilter <- MeanThresholdGeneFilter$new(10)
sut <- DichotomicDGE$new(meanThresholdGeneFilter)

# given
disease_rna_seq_filename <- absolute_path_filename("unit_test_data/voom/GSE92592_raw_counts_GRCh38.p13_NCBI.tsv.gz")
disease_rna_seq <- as.matrix(data.table::fread(disease_rna_seq_filename, header = T, colClasses = "integer"), rownames = "GeneID")
disease_rna_seq_log2 <- log2(disease_rna_seq+1)
disease_rna_seq_sample_01_map <- "111111111111111111110000000000000000000"

expected <- absolute_path_readRDS("unit_test_data/DGE/DichotomicDGE_expected.Rds")

# when
result <- sut$compute(disease_rna_seq_log2, disease_rna_seq_sample_01_map, TRUE)
result$p.value <- NULL
result$adj.p.value <- NULL

# then
test_that("test-DichotomicDGE", {
  expect_equal(result, expected)
}
)

...
# setup
sut <- IlluminaLMMVoomLmerDGE$new()
...
formula <- ~tissue_status + (1 | tissue_status)
tissue_status_field_name <- "tissue_status"
sample_id_field_name <- "accession"
additional_fields <- "tissue"
expected <- package_readRDS("test/voom/IlluminaLMMVoomLmerDGE2_expected.Rds")
...
# then
test_that("test-IlluminaLMMVoomLmerDGE2", {

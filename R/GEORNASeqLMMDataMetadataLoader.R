GEORNASeqLMMDataMetadataLoader <- R6Class(
  ...
  public = list(
    ...
    load = function(rna_seq_data_filename, rna_seq_metadata_filename, random_effect_column_names, sample_status_column_name, sample_statuses_to_be_tested, sample_statuses_map, sample_id_column_name = "accession", additional_columns = NA) {
      ...
      rna_seq_data_metadata <- private$lmmGeneFilter$filter(rna_seq_data, rna_seq_metadata, random_effect_column_names, sample_status_column_name)
      return(
        list(
          metadata = rna_seq_data_metadata$rna_metadata,
          data = rna_seq_data_metadata$rna_data
        )
      )
    }
  ),

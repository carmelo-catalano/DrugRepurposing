IlluminaLMMVoomDGE <- R6Class(
  ...
  public = list(
    ...
    compute = function(rna_seq_metadata_filename, rna_seq_data_filename, formula, tissue_status_field_name, tissue_statuses_to_be_tested, tissue_statuses_map, accession_field_name = "accession", additional_fields = FA, filter_by_protein_coding = F) {
      ...
      rna_seq <- private$illuminaLMMRNASeqLoader$load(rna_seq_metadata_filename, rna_seq_data_filename, tissue_status_field_name, tissue_statuses_to_be_tested, tissue_statuses_map, accession_field_name, additional_fields)

...
IlluminaRNASeqLoader <- R6Class(
  "IlluminaRNASeqLoader",
  public = list(
    initialize = function(geneFilter = NA) {
      ...
      private$illuminaRNASeqMetadataLoader <- IlluminaRNASeqMetadataLoader$new()
    },
    load = function(rna_seq_metadata_filename, rna_seq_data_filename, tissue_statuses_to_be_tested, tissue_statuses_map) {
      rna_seq_metadata <- private$illuminaRNASeqMetadataLoader$load(rna_seq_metadata_filename, tissue_statuses_to_be_tested, tissue_statuses_map)
  ...
  private = list(
    illuminaRNASeqMetadataLoader = NA,
    geneFilter = NA
  )
)
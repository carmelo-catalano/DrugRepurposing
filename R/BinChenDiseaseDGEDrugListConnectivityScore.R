BinChenDiseaseDGEDrugListConnectivityScore <- R6Class(
  ...
  public = list(
    ...
    compute = function(disease_dge, drugs, drugs_genes, disease_n_most_significant_genes, random_distribution_size = 10^5, disease_name = NA, drug_perturbation_time = NA, parallel_computation = F) {
      ...
      return(binChenDiseaseSignatureDrugListConnectivityScore$compute(disease_signature, drugs, length(disease_drug_common_genes), random_distribution_size, disease_name, paste0(disease_n_most_significant_genes, " MostSignificantGenes"), drug_perturbation_time))
    }
  ),
  private = list(

LINCSRDSJuliaLMMDrugDGE <- R6Class(
  ...
  public = list(
    initialize = function(lincs_splitted_level3_dir, output_DGE_dir, BLAS_num_threads = NA, skip_already_computed_genes = F) {
      ...
      juliaLMMDGE <- LINCSJuliaLMMDrugDGEFactory$new()$create(BLAS_num_threads)
      private$lincsLMMDrugDGE <- LINCSLMMDrugDGE$new(lincsMetadataSetuper, lincsRDSDataLoader, juliaLMMDGE, output_DGE_dir, skip_already_computed_genes)
    },
